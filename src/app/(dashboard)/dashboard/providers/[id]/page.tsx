...
function ImportClaudeAuthModal({ onClose, onSuccess }: ImportClaudeAuthModalProps) {
  ...
  const [bulkErrors, setBulkErrors] = useState<
    { index: number; name: string; message: string }[]
  >([]);
  const [bulkResult, setBulkResult] = useState<{
    success: number;
    failed: number;
    ...
    files.forEach((file) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          newEntries.push({ name: file.name.replace(/\.json$/, ""), json, parseError: null, email });
        } catch {
          newEntries.push({ name: file.name, json: null, parseError: "Not valid JSON", email: null });
        }
        pending--;
  ...
  const handleBulkPasteChange = (text: string) => {
    ...
    try {
      ...
      if (Array.isArray(arr)) {
        setBulkEntries(
          arr.map((item, i) => ({ name: `entry ${i + 1}`, json: item, parseError: null, email: null }))
        );
      } else {
        setBulkEntries([{ name: "entry 1", json: arr, parseError: null, email: null }]);
      }
    } catch {
      setBulkEntries([{ name: "parse error", json: null, parseError: "Invalid JSON", email: null }]);
    }
  };
